...
function UserSearch(props) {
  ...
  const setChildrenBig = (e) => {
    ...
    setChildren(finalValue);
    setShowFirst((prev) => prev && childrenC == "" && finalValue < 1);
  };
  ...
  const handlePopoverOpen = (event) => {
    if (childrenC > 0) setAnchorEl(event.currentTarget);
  };
  ...
  function handleSearchFlights() {
    ...
    if (departureAirportC.length !== 0) {
      obj = {
        ...obj,
        ["DepartureAirport"]: departureAirportC.trim().toUpperCase(),
      };

      obj2 = {
        ...obj2,
        ["DepartureAirport"]: arrivalAirportC.trim().toUpperCase(),
      };
    }
    if (arrivalAirportC.length !== 0) {
      obj = {
        ...obj,
        ["ArrivalAirport"]: arrivalAirportC.trim().toUpperCase(),
      };

      obj2 = {
        ...obj2,
        ["ArrivalAirport"]: departureAirportC.trim().toUpperCase(),
      };
    }
      ...
      <div className="cabins">
        ...
        >
          <Checkbox
            ...
            onChange={(e) =>
              setShowFirst(
                e.target.checked && (childrenC == "" || childrenC == 0)
              )
            }
          />
          ...
          >
            <Typography sx={{ p: 1 }}>
              Children are not allowed in First class
            </Typography>
          </Popover>
        </div>
        <div className="cabin">
